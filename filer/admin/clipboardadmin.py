...
def ajax_upload(request, folder_id=None):
    ...
    try:
        if request.POST.get('full_path'):
            sub_folders_list = request.POST['full_path'].split('/')[:-1]
            sub_folder_parent = folder
            for sub_folder_name in sub_folders_list:
                sub_folder, _ = Folder.objects.get_or_create(
                    parent_id=sub_folder_parent.id if sub_folder_parent else None,
                    name=sub_folder_name,
                    owner_id=sub_folder_parent.owner_id if sub_folder_parent else request.user.id
                )
                sub_folder_parent = sub_folder
            folder = sub_folder_parent
