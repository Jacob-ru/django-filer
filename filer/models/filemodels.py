...
class File(PolymorphicModel, mixins.IconsMixin):
    ...
    def edit_url(self):
        url = ''
        if self.file and self.is_public and self.extension in filer_settings.FILER_ALLOWED_FOR_EDIT_EXTENSIONS:
            try:
                url = reverse('file_edit', kwargs={
                    'uploaded_at': self.canonical_time,
                    'file_id': self.id
                })
            except NoReverseMatch:
                pass  # No edit url, return empty string
        return url
